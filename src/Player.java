...
public class Player implements Runnable {
    ...
    public final static Player tcpObj = new Player();
    public final static String END_CHAT_SESSION =
            Character.toString((char) 0); // Indicates the end of a session
    ...
    // Hand
    HashMap<String, ArrayList<Card>> hand = new HashMap<>();
    ...
    public ArrayList<Card> giveAll(Card c) {
        ArrayList<Card> cards = new ArrayList<>(hand.get(c.getRank()));
        if (cards.size() > 0) {
            for (Card card : cards) {
                hand.remove(card.getRank());
            }
        }
        return cards;
    }

    public void addFromDeck(Card c) {
        hand.get(c.getRank()).add(c);
    }

    public void askFor(Card c) {
